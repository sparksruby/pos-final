...
import { pickProductImage, deleteProductImage } from "../../src/utils/productImage";
...
const COMMON_UNITS = ["pcs", "kg", "g", "box", "dozen", "litre", "pack", "bottle"];
...
export default function ProductsManageScreen() {
  ...
  const expiringSoonStr = new Date(Date.now() + EXPIRING_SOON_DAYS * 86400000).toISOString().slice(0, 10);
  ...
  useEffect(() => { loadShopSettings(); initPrinter(); }, []);
  ...
  const visibleCategories = searchQuery.length === 0
    ? categories
    : categories
        .map(cat => ({
          ...cat,
  ...
  useEffect(() => { setVisibleCounts({}); }, [searchQuery]);
  const visibleCountFor = (categoryId: number) => visibleCounts[categoryId] ?? PRODUCTS_PAGE_SIZE;
  const showMoreFor = (categoryId: number) =>
    setVisibleCounts(prev => ({ ...prev, [categoryId]: visibleCountFor(categoryId) + PRODUCTS_PAGE_SIZE }));
  ...
  const openCreateItem = (categoryId: number) => {
    setItemForm({
      mode: "create", categoryId, name: "", sku: "", barcode: "",
      price: "", wholesalePrice: "", costPrice: "0", unit: "pcs",
      imageUri: "", stockQty: "0", lowStockThreshold: "5", expiryDate: "", labelText: "", isActive: true,
    });
  };

  const openEditItem = (item: Product) => {
    setItemForm({
      mode: "edit", categoryId: item.categoryId, productId: item.id,
      name: item.name, sku: item.sku ?? "", barcode: item.barcode ?? "",
      price: String(item.price), wholesalePrice: item.wholesalePrice != null ? String(item.wholesalePrice) : "",
      costPrice: String(item.costPrice), unit: item.unit, imageUri: item.imageUri ?? "",
      stockQty: String(item.stockQty), lowStockThreshold: String(item.lowStockThreshold),
      expiryDate: item.expiryDate ?? "", labelText: item.labelText ?? "", isActive: item.isActive,
    });
  };

  const submitItem = async () => {
    if (!itemForm || !itemForm.name.trim() || !itemForm.price || !currentBranchId) return;
    ...
    const wholesalePrice = wholesalePriceStr ? Number(wholesalePriceStr) : undefined;
    if (wholesalePrice !== undefined && (Number.isNaN(wholesalePrice) || wholesalePrice < 0)) {
    ...
    try {
      if (itemForm.mode === "create") {
        ...
        await productsRepo.createProduct(currentBranchId, {
        ...
        const original = categories
          .find(c => c.id === itemForm.categoryId)?.products
          .find(i => i.id === itemForm.productId);
        await productsRepo.updateProduct(itemForm.productId, currentBranchId, {
          name:       itemForm.name.trim(),
          sku:        itemForm.sku.trim() || undefined,
          barcode:    itemForm.barcode.trim() || undefined,
          price,
  ...
  const toggleAvailable = async (item: Product) => {
    ...
    await productsRepo.updateProduct(item.id, currentBranchId, {
      name: item.name, sku: item.sku, barcode: item.barcode, price: item.price,
      wholesalePrice: item.wholesalePrice, costPrice: item.costPrice, unit: item.unit, imageUri: item.imageUri,
      stockQty: item.stockQty, lowStockThreshold: item.lowStockThreshold, expiryDate: item.expiryDate,
      labelText: item.labelText, isActive: !item.isActive, sortOrder: item.sortOrder,
    });
    reload();
  };
  ...
  const handlePickImage = async () => {
    ...
    try {
      ...
      const message = e?.message === "PERMISSION_DENIED" ? t("products.imagePermission") : t("products.imageFailed");
  ...
  const handleRemoveImage = () => {
    alert(t("products.removePhotoConfirm"), "", [
      { text: t("common.cancel"), style: "cancel" },
      { text: t("common.confirm"), style: "destructive", onPress: () => setItemForm(f => f && { ...f, imageUri: "" }) },
    ]);
  };
  ...
  const openPrintLabel = (item: Product) => setLabelForm({ product: item, qty: "1" });
  ...
  const submitPrintLabel = async () => {
    ...
    try {
      await labelPrinterRef.current?.print({
        ...
        labelText: labelForm.product.labelText || shopSettings?.labelDefaultText || "",
        qty,
      });
      setLabelForm(null);
    } catch (e: any) {
      alert(t("common.error"), e?.message === "NO_PRINTER_SELECTED" ? t("products.noLabelPrinter") : t("products.printLabelFailed"));
    } finally {
  ...
  return (
    <SafeAreaView style={s.root}>
          ...
          {visibleCategories.map(cat => (
            <View key={cat.id} style={s.catCard}>
              <View style={s.catHeader}>
                {canEdit ? (
                  <TouchableOpacity style={s.catNameRow} onPress={() => openEditCategory(cat)}>
                ...
                {canEdit && (
                  <View style={s.catHeaderActions}>
                    <TouchableOpacity style={s.addItemBtn} onPress={() => openCreateItem(cat.id)}>
                      <Ionicons name="add" size={13} color={C.muted} />
                      <Text style={s.addItemBtnText}>{t("products.addProduct")}</Text>
                    </TouchableOpacity>
                    <TouchableOpacity style={s.catDeleteBtn} onPress={() => deleteCategory(cat)}>
              ...
              {cat.products.length === 0 ? (
                ...
                <View key={item.id} style={[s.itemRow, !item.isActive && { opacity: 0.5 }]}>
                  {item.imageUri ? (
                    <Image source={{ uri: item.imageUri }} style={s.itemThumb} />
                  ) : (
                    <View style={s.itemThumbPlaceholder}>
                      <Ionicons name="cube-outline" size={16} color={C.muted} />
                    </View>
                  )}
                  <TouchableOpacity style={{ flex: 1 }} onPress={() => canEdit && openEditItem(item)} disabled={!canEdit}>
                    <Text style={s.itemName}>{item.name}</Text>
                    <Text style={s.itemPrice}>
                      ${item.price.toLocaleString()} / {item.unit}
                      {item.wholesalePrice != null ? ` · ${t("products.wholesalePrice")}: $${item.wholesalePrice.toLocaleString()}` : ""}
                      {" · "}{t("products.stock")}: {item.stockQty}
                    </Text>
                    {item.stockQty <= 0 ? (
                      <Text style={s.stockBadgeDanger}>{t("inventory.outOfStock")}</Text>
                    ) : item.stockQty <= item.lowStockThreshold ? (
                      <Text style={s.stockBadgeWarning}>{t("inventory.lowStock")}</Text>
                    ) : null}
                    {!!item.expiryDate && item.expiryDate < todayStr ? (
                      <Text style={s.stockBadgeDanger}>{t("alerts.expired")}</Text>
                    ) : !!item.expiryDate && item.expiryDate <= expiringSoonStr ? (
                      <Text style={s.stockBadgeWarning}>{t("alerts.expiringSoon")}</Text>
                    ) : null}
                  </TouchableOpacity>
                  {canEdit && (
                    <>
                      <Switch
                        ...
                        thumbColor="#fff"
                      />
                      <TouchableOpacity style={s.deleteBtn} onPress={() => openPrintLabel(item)}>
                        <Ionicons name="pricetag-outline" size={14} color={C.textSub} />
                      </TouchableOpacity>
              ...
              {cat.products.length > visibleCountFor(cat.id) && (
                <TouchableOpacity style={s.loadMoreBtn} onPress={() => showMoreFor(cat.id)}>
                  <Text style={s.loadMoreBtnText}>
                    {t("common.loadMore")} ({cat.products.length - visibleCountFor(cat.id)})
                  </Text>
                </TouchableOpacity>
              )}
            </View>
      ...
      <Modal visible={!!catForm} animationType="slide" transparent onRequestClose={() => setCatForm(null)}>
        <View style={s.modalOverlay}>
          {catForm && (
            <View style={s.modalSheet}>
              <Text style={s.modalTitle}>
                {catForm.mode === "create" ? t("products.newCategoryTitle") : t("products.renameCategoryTitle")}
              </Text>
              <TextInput
                style={s.input}
                value={catForm.name}
              ...
              <View style={s.modalActions}>
                <TouchableOpacity style={s.cancelBtn} onPress={() => setCatForm(null)}>
      ...
      {/* Product modal */}
      <Modal visible={!!itemForm} animationType="slide" transparent onRequestClose={() => setItemForm(null)}>
        <View style={s.modalOverlay}>
          {itemForm && (
            <ScrollView style={s.modalSheet} keyboardShouldPersistTaps="handled">
              <Text style={s.modalTitle}>
                {itemForm.mode === "create" ? t("products.addProductTitle") : t("products.editProductTitle")}
              </Text>

              <Text style={s.label}>{t("products.image")}</Text>
              <View style={s.photoRow}>
                {itemForm.imageUri ? (
                  <Image source={{ uri: itemForm.imageUri }} style={s.photoPreview} />
                ) : (
                ...
                <View style={{ flex: 1, gap: 8 }}>
                  <TouchableOpacity style={s.photoBtn} onPress={handlePickImage} disabled={imagePicking}>
                    {imagePicking
                      ...
                      : (
                        <>
                          ...
                          <Text style={s.photoBtnText}>
                            {itemForm.imageUri ? t("products.changePhoto") : t("products.addPhoto")}
                          </Text>
                        </>
                      )
                    }
                  ...
                  {itemForm.imageUri && (
                    <TouchableOpacity style={s.photoRemoveBtn} onPress={handleRemoveImage}>
                      <Ionicons name="trash-outline" size={15} color={C.danger} />
                      <Text style={s.photoRemoveBtnText}>{t("products.removePhoto")}</Text>
                    </TouchableOpacity>
                  )}
                </View>
              </View>

              <Text style={[s.label, { marginTop: 12 }]}>{t("products.name")}</Text>
              <TextInput
                style={s.input}
                value={itemForm.name}
              ...
              <Text style={[s.label, { marginTop: 12 }]}>{t("products.sku")}</Text>
              <TextInput
                style={s.input}
                value={itemForm.sku}
              ...
              <Text style={[s.label, { marginTop: 12 }]}>{t("products.barcode")}</Text>
              <TextInput
                style={s.input}
                value={itemForm.barcode}
                onChangeText={v => setItemForm(f => f && { ...f, barcode: v })}
                placeholderTextColor={C.muted}
              ...
              <View style={s.row2}>
                <View style={{ flex: 1 }}>
                  <Text style={[s.label, { marginTop: 12 }]}>{t("products.price")}</Text>
                  <TextInput
                    style={s.input}
                    value={itemForm.price}
                    onChangeText={v => setItemForm(f => f && { ...f, price: v })}
                    keyboardType="numeric"
                    placeholderTextColor={C.muted}
                  />
                </View>
                <View style={{ flex: 1 }}>
                  <Text style={[s.label, { marginTop: 12 }]}>{t("products.costPrice")}</Text>
                  <TextInput
                    style={s.input}
                    value={itemForm.costPrice}
                    onChangeText={v => setItemForm(f => f && { ...f, costPrice: v })}
                    keyboardType="numeric"
                    placeholderTextColor={C.muted}
                  />
                </View>
              ...
              <View style={s.row2}>
                <View style={{ flex: 1 }}>
                  <Text style={[s.label, { marginTop: 12 }]}>{t("products.wholesalePrice")}</Text>
                  <TextInput
                    style={s.input}
                    value={itemForm.wholesalePrice}
                    onChangeText={v => setItemForm(f => f && { ...f, wholesalePrice: v })}
                    keyboardType="numeric"
                ...
                <View style={{ flex: 1 }}>
                  <Text style={[s.label, { marginTop: 12 }]}>{t("products.unit")}</Text>
                  <TextInput
                    style={s.input}
                    value={itemForm.unit}
                    onChangeText={v => setItemForm(f => f && { ...f, unit: v })}
                    placeholder="pcs"
                    placeholderTextColor={C.muted}
                    autoCapitalize="none"
                  />
              ...
              <View style={s.unitChipsRow}>
                {COMMON_UNITS.map(u => (
                  <TouchableOpacity
                    key={u}
                    style={[s.unitChip, itemForm.unit === u && s.unitChipActive]}
                    onPress={() => setItemForm(f => f && { ...f, unit: u })}
                  >
                    <Text style={[s.unitChipText, itemForm.unit === u && s.unitChipTextActive]}>{u}</Text>
                  </TouchableOpacity>
                ))}
              </View>
              ...
              <View style={s.row2}>
                <View style={{ flex: 1 }}>
                  <Text style={[s.label, { marginTop: 12 }]}>
                    {t("products.stock")}{branches.length > 1 && currentBranch ? ` (${currentBranch.name})` : ""}
                  </Text>
                  <TextInput
                    style={s.input}
                    value={itemForm.stockQty}
                    onChangeText={v => setItemForm(f => f && { ...f, stockQty: v })}
                    keyboardType="numeric"
                    placeholderTextColor={C.muted}
                  />
                </View>
                <View style={{ flex: 1 }}>
                  <Text style={[s.label, { marginTop: 12 }]}>{t("inventory.lowStockThreshold")}</Text>
                  <TextInput
                    style={s.input}
                    value={itemForm.lowStockThreshold}
                    onChangeText={v => setItemForm(f => f && { ...f, lowStockThreshold: v })}
                    keyboardType="numeric"
                    placeholderTextColor={C.muted}
                  />
                </View>
              ...
              <Text style={[s.label, { marginTop: 12 }]}>{t("products.expiryDate")}</Text>
              <TouchableOpacity style={s.input} onPress={() => setDatePickerOpen(true)} activeOpacity={0.7}>
                <Text style={itemForm.expiryDate ? s.dateValueText : s.dateValuePlaceholder}>
                  {itemForm.expiryDate || t("products.expiryDatePlaceholder")}
                </Text>
              </TouchableOpacity>

              <Text style={[s.label, { marginTop: 12 }]}>{t("products.labelText")}</Text>
              <TextInput
                style={s.input}
                value={itemForm.labelText}
                onChangeText={v => setItemForm(f => f && { ...f, labelText: v })}
                placeholder={shopSettings?.labelDefaultText || t("products.labelTextPlaceholder")}
                placeholderTextColor={C.muted}
              />
              ...
              {itemForm.mode === "edit" && (
                <View style={s.availRow}>
                  ...
                  <Switch
                    ...
                    onValueChange={v => setItemForm(f => f && { ...f, isActive: v })}
                    trackColor={{ false: C.border, true: C.accent }}
                    thumbColor="#fff"
                  />
                </View>
              ...
              <View style={s.modalActions}>
                <TouchableOpacity style={s.cancelBtn} onPress={() => setItemForm(null)}>
                  <Text style={s.cancelBtnText}>{t("common.cancel")}</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  style={[s.saveBtn, (!itemForm.name.trim() || !itemForm.price) && s.saveBtnOff]}
                  onPress={submitItem}
                  disabled={!itemForm.name.trim() || !itemForm.price || itemSubmitting}
                >
                  {itemSubmitting
                    ? <ActivityIndicator color={C.accentFg} />
                    : <Text style={s.saveBtnText}>{t("common.save")}</Text>
      ...
      {/* Print label modal */}
      <Modal visible={!!labelForm} animationType="slide" transparent onRequestClose={() => setLabelForm(null)}>
        <View style={s.modalOverlay}>
          {labelForm && (
            <View style={s.modalSheet}>
              ...
              {!labelForm.product.barcode && (
                <Text style={s.labelNoBarcodeHint}>{t("products.labelNoBarcodeHint")}</Text>
              )}

              <Text style={[s.label, { marginTop: 12 }]}>{t("products.labelQty")}</Text>
              <TextInput
                style={s.input}
                value={labelForm.qty}
              ...
              <View style={s.modalActions}>
                <TouchableOpacity style={s.cancelBtn} onPress={() => setLabelForm(null)}>
                  <Text style={s.cancelBtnText}>{t("common.cancel")}</Text>
                </TouchableOpacity>
                <TouchableOpacity style={s.saveBtn} onPress={submitPrintLabel} disabled={labelPrinting}>
                  {labelPrinting
                    ? <ActivityIndicator color={C.accentFg} />
                    : <Text style={s.saveBtnText}>{t("products.printLabelAction")}</Text>
                  }
                </TouchableOpacity>
              </View>
            </View>
...
const makeStyles = (C: ThemeColors, isTablet: boolean) => StyleSheet.create({
  ...
  catCard: { backgroundColor: C.surface, borderRadius: R.lg, padding: 14,
  ...
  catNameRow: { flexDirection: "row", alignItems: "center", gap: 6, flexShrink: 1 },
  catName: { color: C.text, fontSize: F.lg, fontWeight: "800", flexShrink: 1 },
  ...
  catDeleteBtn: { width: 30, height: 30, borderRadius: R.md, backgroundColor: C.card,
                  alignItems: "center", justifyContent: "center",
                  borderWidth: 1, borderColor: C.border },

  noItemsText: { color: C.muted, fontSize: F.xs, paddingVertical: 10, textAlign: "center" },

  loadMoreBtn: { marginTop: 6, paddingVertical: 10, borderRadius: R.md, backgroundColor: C.card,
  ...
  itemThumb: { width: 40, height: 40, borderRadius: R.md, backgroundColor: C.card },
  itemThumbPlaceholder: { width: 40, height: 40, borderRadius: R.md, backgroundColor: C.card,
                           alignItems: "center", justifyContent: "center",
                           borderWidth: 1, borderColor: C.border },
  itemName:  { color: C.text, fontSize: F.sm, fontWeight: "600" },
  itemPrice: { color: C.accent, fontSize: F.sm, fontWeight: "700", marginTop: 2 },
  stockBadgeDanger:  { color: C.danger, fontSize: F.xs, fontWeight: "700", marginTop: 2 },
  stockBadgeWarning: { color: C.warning, fontSize: F.xs, fontWeight: "700", marginTop: 2 },
  deleteBtn: { width: 32, height: 32, borderRadius: R.md, backgroundColor: C.card,
  ...
  unitChipsRow: { flexDirection: "row", flexWrap: "wrap", gap: 6, marginTop: 8 },
  unitChip: { paddingHorizontal: 10, paddingVertical: 5, borderRadius: R.full,
  ...
  photoPreview: { width: 64, height: 64, borderRadius: R.md, backgroundColor: C.card },
  photoPlaceholder: { width: 64, height: 64, borderRadius: R.md, backgroundColor: C.card,
                       alignItems: "center", justifyContent: "center",
                       borderWidth: 1, borderColor: C.border },
  photoBtn: { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 6,
              backgroundColor: C.card, paddingVertical: 9, borderRadius: R.md,
              borderWidth: 1, borderColor: C.border },
  photoBtnText: { color: C.text, fontSize: F.xs, fontWeight: "700" },
  photoRemoveBtn: { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 6,
                    paddingVertical: 6 },
