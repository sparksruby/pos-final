...
interface Row { label: string; qty: number; revenue: number; profit?: number; }

const addRow = (map: Map<string, Row>, key: string, qty: number, revenue: number, profit?: number) => {
  const existing = map.get(key) ?? { label: key, qty: 0, revenue: 0, profit: 0 };
  existing.qty += qty;
...
export default function ReportsScreen() {
  ...
  const sales = branchFilter === "all" ? allSales : allSales.filter(sale => sale.branchId === branchFilter);
  ...
  const [productMeta, setProductMeta] = useState<Map<number, { category: string; costPrice: number }>>(new Map());
  ...
  const report = useMemo(() => {
    ...
    for (const sale of sales) {
      ...
      if (sale.branchName) addRow(byBranch, sale.branchName, 1, sale.total - refunded);
      ...
      for (const item of sale.items) {
        ...
        addRow(byCategory, meta?.category ?? t("reports.uncategorized"), netQty, netRevenue, profit);
      }
    }

    const netRevenue = grossRevenue - refundedTotal;
    const otherDiscountTotal = Math.max(0, sales.reduce((s, sale) => s + sale.discount, 0) - itemDiscountTotal);
    const totalDiscount = itemDiscountTotal + otherDiscountTotal;

    const sortDesc = (m: Map<string, Row>) => Array.from(m.values()).sort((a, b) => b.revenue - a.revenue);

    return {
      count: sales.length,
      netRevenue, refundedTotal, profitTotal,
      itemDiscountTotal, otherDiscountTotal, totalDiscount,
  ...
  return (
    <SafeAreaView style={s.root}>
      ...
      {branches.length > 1 && (
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ flexGrow: 0 }} contentContainerStyle={s.branchFilterRow}>
          <TouchableOpacity
            style={[s.branchFilterBtn, branchFilter === "all" && s.branchFilterBtnActive]}
            onPress={() => setBranchFilter("all")}
          >
            <Text style={[s.branchFilterText, branchFilter === "all" && s.branchFilterTextActive]}>
          ...
          {branches.map(b => (
            <TouchableOpacity
              key={b.id}
              style={[s.branchFilterBtn, branchFilter === b.id && s.branchFilterBtnActive]}
              onPress={() => setBranchFilter(b.id)}
            >
              <Text style={[s.branchFilterText, branchFilter === b.id && s.branchFilterTextActive]} numberOfLines={1}>
                {b.name}
              </Text>
            </TouchableOpacity>
          ))}
      ...
      {isLoading ? (
        ...
        <ScrollView contentContainerStyle={s.scroll}>
          ...
          <View style={s.statGrid}>
            <StatCard s={s} C={C} label={t("reports.sales")}   value={String(report.count)} />
            <StatCard s={s} label={t("reports.revenue")} value={`${currency}${report.netRevenue.toLocaleString()}`} accent C={C} />
            {report.refundedTotal > 0 &&
              <StatCard s={s} label={t("reports.refunded")} value={`-${currency}${report.refundedTotal.toLocaleString()}`} danger C={C} />}
            {report.totalDiscount > 0 &&
              <StatCard s={s} label={t("reports.discountsGiven")} value={`-${currency}${report.totalDiscount.toLocaleString()}`} danger C={C} />}
            <StatCard s={s} label={t("reports.profit")} value={`${currency}${report.profitTotal.toLocaleString()}`} accent C={C} />
          </View>
            ...
            <ReportSection s={s} C={C} title={t("reports.byBranch")}>
              {report.byBranch.map(row => (
                <View key={row.label} style={s.row}>
                  ...
                  <Text style={s.rowSub}>{t("reports.salesCount", { count: row.qty })}</Text>
                  <Text style={s.rowValue}>{currency}{row.revenue.toLocaleString()}</Text>
                </View>
              ))}
            </ReportSection>
          )}
          ...
          <ReportSection s={s} C={C} title={t("reports.byPaymentMethod")}>
            {report.byMethod.map(row => (
              <View key={row.label} style={s.row}>
                <View style={s.rowLeft}>
                  <Ionicons name={METHOD_ICONS[row.label] ?? "cash-outline"} size={14} color={C.textSub} />
                  <Text style={s.rowLabel}>{row.label}</Text>
                </View>
                <Text style={s.rowSub}>{t("reports.salesCount", { count: row.qty })}</Text>
                <Text style={s.rowValue}>{currency}{row.revenue.toLocaleString()}</Text>
          ...
          <ReportSection s={s} C={C} title={t("reports.byCashier")}>
            {report.byCashier.map(row => (
              <View key={row.label} style={s.row}>
                <Text style={[s.rowLabel, { flex: 1 }]} numberOfLines={1}>{row.label}</Text>
                <Text style={s.rowSub}>{t("reports.salesCount", { count: row.qty })}</Text>
                <Text style={s.rowValue}>{currency}{row.revenue.toLocaleString()}</Text>
              </View>
            ))}
          ...
          <ReportSection s={s} C={C} title={t("reports.byCategory")}>
            {report.byCategory.map(row => (
              <View key={row.label} style={s.row}>
                <Text style={[s.rowLabel, { flex: 1 }]} numberOfLines={1}>{row.label}</Text>
                <Text style={s.rowSub}>{t("reports.qtySold", { qty: row.qty })}</Text>
                <Text style={s.rowValue}>{currency}{row.revenue.toLocaleString()}</Text>
              </View>
            ))}
          </ReportSection>
          ...
          <ReportSection s={s} C={C} title={t("reports.discountBreakdown")}>
            <View style={s.row}>
              <Text style={[s.rowLabel, { flex: 1 }]}>{t("reports.itemDiscounts")}</Text>
              <Text style={s.rowValue}>{currency}{report.itemDiscountTotal.toLocaleString()}</Text>
            </View>
            <View style={s.row}>
              <Text style={[s.rowLabel, { flex: 1 }]}>{t("reports.orderDiscounts")}</Text>
              <Text style={s.rowValue}>{currency}{report.otherDiscountTotal.toLocaleString()}</Text>
            </View>
          </ReportSection>
            ...
            {stockOnHand.length === 0 ? (
              ...
              return (
                <View key={p.id} style={s.row}>
                  <Text style={[s.rowLabel, { flex: 1 }]} numberOfLines={1}>{p.name}</Text>
                  {outOfStock && <Text style={s.stockBadgeDanger}>{t("inventory.outOfStock")}</Text>}
                  {lowStock && <Text style={s.stockBadgeWarning}>{t("inventory.lowStock")}</Text>}
                  <Text style={[s.rowValue, outOfStock && s.stockBadgeDanger]}>{p.stockQty} {p.unit}</Text>
                </View>
              );
            })}
          </ReportSection>
...
const StatCard = ({
  s, C, label, value, accent, danger,
}: { s: Styles; C: ThemeColors; label: string; value: string; accent?: boolean; danger?: boolean }) => (
  <View style={s.statCard}>
    <Text style={[s.statValue, accent && { color: C.accent }, danger && { color: C.danger }]} numberOfLines={1}>
      {value}
    </Text>
...
const ReportSection = ({
  s, C, title, children,
}: { s: Styles; C: ThemeColors; title: string; children: React.ReactNode }) => {
...
const makeStyles = (C: ThemeColors, isTablet: boolean) => StyleSheet.create({
  ...
  periodRow: { flexDirection: "row", flexWrap: "wrap", gap: 8, paddingHorizontal: 16, marginBottom: 8 },
  periodBtn: { flexGrow: 1, flexBasis: "22%", paddingVertical: 10, borderRadius: R.md, backgroundColor: C.card,
  ...
  branchFilterRow: { paddingHorizontal: 16, marginBottom: 8, gap: 8, flexDirection: "row" },
  branchFilterBtn: { paddingHorizontal: 14, paddingVertical: 8, borderRadius: R.full,
  ...
  navLabel: { color: C.text, fontSize: F.md, fontWeight: "700", minWidth: 160, textAlign: "center" },
  ...
  statCard: { flexGrow: 1, flexBasis: "31%", backgroundColor: C.surface, borderRadius: R.lg, padding: 14,
              borderWidth: 1, borderColor: C.border, alignItems: "center", ...Shadow.sm },
  statValue: { color: C.text, fontSize: F.lg, fontWeight: "800" },
  statLabel: { color: C.muted, fontSize: F.xs, marginTop: 4, textAlign: "center" },
  profitHint: { color: C.muted, fontSize: F.xs, marginTop: -2, marginBottom: 2 },

  card: { backgroundColor: C.surface, borderRadius: R.lg, padding: 16,
          borderWidth: 1, borderColor: C.border, gap: 2, ...Shadow.sm },
  ...
  rowValue: { color: C.accent, fontSize: F.sm, fontWeight: "700", marginLeft: "auto" },
