...
interface DeviceInfoDto {
  deviceId: string; deviceName: string; branchId: string; branchName: string; lastSyncAt: string | null;
}
interface BranchDto { id: string; name: string; address: string | null; phone: string | null; isActive: boolean; updatedAt: string; }
interface CategoryDto { id: string; name: string; sortOrder: number; isActive: boolean; updatedAt: string; }
interface ProductDto {
  id: string; categoryId: string; name: string; sku: string | null; barcode: string | null;
  ...
  price: number; wholesalePrice?: number | null; costPrice: number; unit?: string | null;
  imageUri: string | null; lowStockThreshold: number;
  expiryDate: string | null; isActive: boolean; sortOrder: number; updatedAt: string;
}
interface BranchStockDto { branchId: string; productId: string; stockQty: number; updatedAt: string; }
interface SaleItemDto {
  id: string; productId: string; productName: string; unitPrice: number; qty: number; subtotal: number; discount: number;
}
interface SaleDto {
  id: string; branchId: string; subtotal: number; discount: number; taxPercent: number; taxAmount: number;
  total: number; tendered: number; changeDue: number; paymentMethod: string;
  cashierName: string | null; customerName: string | null; createdAt: string; items: SaleItemDto[];
}
interface StockMovementDto {
  id: string; productId: string; branchId: string; changeQty: number; reason: string;
  resultingStockQty: number; actorName: string | null; createdAt: string;
}
interface ExpenseDto {
  id: string; branchId: string; category: string; description: string | null;
  amount: number; actorName: string | null; createdAt: string;
}
...
interface CategoryPushDto { id: string; name: string; sortOrder: number; isActive: boolean; }
interface ProductPushDto {
  id: string; categoryId: string; name: string; sku: string | null; barcode: string | null;
  price: number; wholesalePrice: number | null; costPrice: number; unit: string;
  imageUri: string | null; lowStockThreshold: number;
  expiryDate: string | null; isActive: boolean; sortOrder: number;
}
interface PullResponse {
  serverTime: string; branches: BranchDto[]; categories: CategoryDto[]; products: ProductDto[]; branchStock: BranchStockDto[];
  sales: SaleDto[]; stockMovements: StockMovementDto[]; expenses: ExpenseDto[];
}
// Push-direction shape for a branch created offline — same minimal,
// no-updatedAt idea as CategoryPushDto/ProductPushDto below.
interface BranchPushDto { id: string; name: string; address: string | null; phone: string | null; isActive: boolean; }
...
interface AcceptedIdDto { clientId: string; serverId: string; }
interface PushResponse {
  acceptedBranches: AcceptedIdDto[]; acceptedCategories: AcceptedIdDto[]; acceptedProducts: AcceptedIdDto[];
  acceptedSaleIds: string[]; acceptedMovementIds: string[]; acceptedExpenseIds: string[]; updatedStock: BranchStockDto[];
}
...
export interface RemoteSale {
  ...
  createdAt:     string;
  items:         { productName: string; unitPrice: number; qty: number; subtotal: number }[];
}
...
interface ConfigRow {
  server_url: string | null; device_api_key: string | null; bound_branch_id: number | null;
  last_pull_at: string | null; last_push_at: string | null; admin_key: string | null;
}

const getConfig = async (): Promise<SyncConfig | null> => {
  const db = await getDb();
  const row = await db.getFirstAsync<ConfigRow>("SELECT * FROM sync_config WHERE id = 1");
  if (!row || !row.server_url || !row.device_api_key || !row.bound_branch_id) return null;
  return {
    serverUrl: row.server_url, deviceApiKey: row.device_api_key, boundBranchId: row.bound_branch_id,
    lastPullAt: row.last_pull_at, lastPushAt: row.last_push_at, adminKey: row.admin_key,
  };
};
...
const fetchJson = async <T>(url: string, apiKey: string, init?: RequestInit): Promise<T> => {
  ...
  try {
    res = await fetch(url, {
      ...init,
      headers: { "X-Device-Key": apiKey, "Content-Type": "application/json", ...(init?.headers ?? {}) },
    });
  } catch (e: any) {
...
const getServerId = async (
  db: Awaited<ReturnType<typeof getDb>>, table: "branches" | "categories" | "products", localId: number
): Promise<string | null> => {
  const row = await db.getFirstAsync<{ server_id: string | null }>(
    `SELECT server_id FROM ${table} WHERE id = ?`, [localId]
  );
...
const idFor = async (
  ...
  const row = await db.getFirstAsync<{ server_id: string | null; sync_uuid: string | null }>(
    `SELECT server_id, sync_uuid FROM ${table} WHERE id = ?`, [localId]
  );
  return row?.server_id ?? row?.sync_uuid ?? null;
};

const fetchJsonAdmin = async <T>(url: string, adminKey: string, init?: RequestInit): Promise<T> => {
  const method = init?.method ?? "GET";
  logRequest(method, url);
  let res: Response;
  try {
...
export const syncRepo = {
  ...
  connect: async (
    serverUrlInput: string, deviceApiKey: string, adoptLocalBranchId?: number | null
  ): Promise<{ branchId: number; branchName: string }> => {
    const serverUrl = serverUrlInput.trim().replace(/\/+$/, "");
    if (!serverUrl || !deviceApiKey.trim()) throw new SyncError("MISSING_CONFIG");

    const me = await fetchJson<DeviceInfoDto>(`${serverUrl}/api/sync/me`, deviceApiKey);

    const db = await getDb();
    let branchId = 0;
    await db.withTransactionAsync(async () => {
      const existing = await db.getFirstAsync<{ id: number }>(
        "SELECT id FROM branches WHERE server_id = ?", [me.branchId]
      );
      if (existing) {
        ...
        const adoptee = adoptLocalBranchId != null
          ? await db.getFirstAsync<{ id: number; server_id: string | null }>(
              "SELECT id, server_id FROM branches WHERE id = ?", [adoptLocalBranchId]
            )
          : null;
  ...
  pull: async (): Promise<{
    branches: number; categories: number; products: number; stockRows: number;
    salesReceived: number; movementsReceived: number; expensesReceived: number;
  }> => {
    ...
    const resp = await fetchJson<PullResponse>(
      `${config.serverUrl}/api/sync/pull?since=${encodeURIComponent(since)}`, config.deviceApiKey
    );
    ...
    await db.withTransactionAsync(async () => {
      ...
      for (const p of resp.products) {
        const category = await db.getFirstAsync<{ id: number }>(
          "SELECT id FROM categories WHERE server_id = ?", [p.categoryId]
        );
        ...
        await db.runAsync(
          ...
          [category.id, p.name, p.sku, p.barcode, p.price, p.wholesalePrice ?? null,
           p.costPrice, p.unit ?? null, p.imageUri,
           p.lowStockThreshold, p.expiryDate, p.isActive ? 1 : 0, p.sortOrder, p.id]
        );
      ...
      for (const bs of resp.branchStock) {
        const branch = await db.getFirstAsync<{ id: number }>("SELECT id FROM branches WHERE server_id = ?", [bs.branchId]);
        const product = await db.getFirstAsync<{ id: number }>("SELECT id FROM products WHERE server_id = ?", [bs.productId]);
        ...
        await db.runAsync(
          ...
          [branch.id, product.id, bs.stockQty, thresholdByServerId.get(bs.productId) ?? null]
        );
      }
      ...
      if (config.adminKey) {
        for (const saleDto of resp.sales) {
          const existingSale = await db.getFirstAsync<{ id: number }>(
            "SELECT id FROM remote_sales WHERE server_sale_id = ?", [saleDto.id]
          );
          if (existingSale) continue;

          const branch = await db.getFirstAsync<{ name: string }>(
            "SELECT name FROM branches WHERE server_id = ?", [saleDto.branchId]
          );
          ...
          const { lastInsertRowId: remoteSaleId } = await db.runAsync(
            `INSERT INTO remote_sales
             ...
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [saleDto.id, branch?.name ?? saleDto.branchId, saleDto.subtotal, saleDto.discount, saleDto.taxPercent,
             saleDto.taxAmount, saleDto.total, saleDto.tendered, saleDto.changeDue, saleDto.paymentMethod,
             saleDto.cashierName, saleDto.customerName, saleDto.createdAt]
          );
          for (const item of saleDto.items) {
            await db.runAsync(
              `INSERT INTO remote_sale_items (remote_sale_id, product_name, unit_price, qty, subtotal, discount)
               VALUES (?, ?, ?, ?, ?, ?)`,
              [remoteSaleId, item.productName, item.unitPrice, item.qty, item.subtotal, item.discount]
            );
          }
          salesReceived++;
        }
        ...
        for (const m of resp.stockMovements) {
          const existingMovement = await db.getFirstAsync<{ id: number }>(
            "SELECT id FROM remote_stock_movements WHERE server_movement_id = ?", [m.id]
          );
          if (existingMovement) continue;

          const branch = await db.getFirstAsync<{ name: string }>(
            "SELECT name FROM branches WHERE server_id = ?", [m.branchId]
          );
          const product = await db.getFirstAsync<{ name: string }>(
            "SELECT name FROM products WHERE server_id = ?", [m.productId]
          );
          ...
          await db.runAsync(
            `INSERT INTO remote_stock_movements
             ...
             VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
            [m.id, product?.name ?? m.productId, branch?.name ?? m.branchId, m.changeQty, m.reason,
             m.resultingStockQty, m.actorName, m.createdAt]
          );
          movementsReceived++;
        }
        ...
        for (const e of resp.expenses) {
          const existingExpense = await db.getFirstAsync<{ id: number }>(
            "SELECT id FROM remote_expenses WHERE server_expense_id = ?", [e.id]
          );
          if (existingExpense) continue;

          const branch = await db.getFirstAsync<{ name: string }>(
            "SELECT name FROM branches WHERE server_id = ?", [e.branchId]
          );
          ...
          await db.runAsync(
            `INSERT INTO remote_expenses
             ...
             VALUES (?, ?, ?, ?, ?, ?, ?)`,
            [e.id, e.category, e.description, e.amount, branch?.name ?? e.branchId, e.actorName, e.createdAt]
          );
          expensesReceived++;
        }
      }

      await db.runAsync("UPDATE sync_config SET last_pull_at = ? WHERE id = 1", [resp.serverTime]);
    });

    return {
      branches: resp.branches.length, categories: resp.categories.length,
      products: resp.products.length, stockRows: resp.branchStock.length,
      salesReceived, movementsReceived, expensesReceived,
    };
  },
  ...
  push: async (): Promise<{ salesPushed: number; movementsPushed: number; skipped: number }> => {
    const config = await getConfig();
    if (!config) throw new SyncError("NOT_CONNECTED");
    ...
    const branchRows = await db.getAllAsync<{
      id: number; name: string; address: string | null; phone: string | null; is_active: number; sync_uuid: string;
    ...
    const branches: BranchPushDto[] = branchRows.map(b => ({
      id: b.sync_uuid, name: b.name, address: b.address, phone: b.phone, isActive: !!b.is_active,
    }));

    const categoryRows = await db.getAllAsync<{
      id: number; name: string; sort_order: number; is_active: number; sync_uuid: string;
    ...
    const categories: CategoryPushDto[] = categoryRows.map(c => ({
      id: c.sync_uuid, name: c.name, sortOrder: c.sort_order, isActive: !!c.is_active,
    }));
    ...
    const localBranch = await db.getFirstAsync<{ id: number }>(
      "SELECT id FROM branches WHERE server_id = ?", [config.boundBranchId]
    );
    const productRows = await db.getAllAsync<{
      id: number; category_id: number; name: string; sku: string | null; barcode: string | null;
      price: number; wholesale_price: number | null; cost_price: number; unit: string;
      image_uri: string | null; low_stock_threshold: number;
      expiry_date: string | null; is_active: number; sort_order: number; sync_uuid: string;
               ...
               p.expiry_date, p.is_active, p.sort_order, p.sync_uuid
        FROM products p WHERE p.server_id IS NULL AND p.sync_uuid IS NOT NULL`,
       [localBranch?.id ?? -1]);
    const products: ProductPushDto[] = [];
    for (const p of productRows) {
      const categoryId = await idFor(db, "categories", p.category_id);
      if (!categoryId) { skipped++; continue; } // its category has no id at all yet — shouldn't happen, see comment above
      products.push({
        id: p.sync_uuid, categoryId, name: p.name, sku: p.sku, barcode: p.barcode,
        price: p.price, wholesalePrice: p.wholesale_price, costPrice: p.cost_price,
        unit: p.unit, imageUri: p.image_uri,
        lowStockThreshold: p.low_stock_threshold, expiryDate: p.expiry_date,
    ...
    const saleRows = await db.getAllAsync<{
      id: number; subtotal: number; discount: number; tax_percent: number; tax_amount: number;
      total: number; tendered: number; change_due: number; payment_method: string;
      cashier_name: string | null; customer_name: string | null; branch_id: number;
      sync_uuid: string; created_at: string;
    ...
    for (const s of saleRows) {
      ...
      const itemRows = await db.getAllAsync<{ product_id: number; product_name: string; unit_price: number; qty: number; subtotal: number; discount: number }>(
        "SELECT product_id, product_name, unit_price, qty, subtotal, discount FROM sale_items WHERE sale_id = ?", [s.id]
      );
      const items: SaleItemDto[] = [];
      let itemsOk = true;
      for (const i of itemRows) {
        const productId = await idFor(db, "products", i.product_id);
        if (!productId) { itemsOk = false; break; }
        items.push({
          id: Crypto.randomUUID(), productId, productName: i.product_name,
          unitPrice: i.unit_price, qty: i.qty, subtotal: i.subtotal, discount: i.discount,
        });
      }
      if (!itemsOk) { skipped++; continue; }

      sales.push({
        id: s.sync_uuid, branchId, subtotal: s.subtotal, discount: s.discount,
        taxPercent: s.tax_percent, taxAmount: s.tax_amount, total: s.total, tendered: s.tendered,
        changeDue: s.change_due, paymentMethod: s.payment_method, cashierName: s.cashier_name,
        customerName: s.customer_name, createdAt: s.created_at, items,
      });
    }

    const movementRows = await db.getAllAsync<{
      id: number; product_id: number; branch_id: number; change_qty: number; reason: string;
      resulting_stock_qty: number; actor_name: string | null; sync_uuid: string; created_at: string;
    ...
    for (const m of movementRows) {
      ...
      if (!branchId || !productId) { skipped++; continue; }
      movements.push({
        id: m.sync_uuid, productId, branchId, changeQty: m.change_qty,
        reason: m.reason, resultingStockQty: m.resulting_stock_qty, actorName: m.actor_name, createdAt: m.created_at,
      });
    }

    const expenseRows = await db.getAllAsync<{
      id: number; category: string; description: string | null; amount: number;
      branch_id: number; actor_name: string | null; sync_uuid: string; created_at: string;
    ...
    for (const e of expenseRows) {
      ...
      if (!branchId) { skipped++; continue; }
      expenses.push({
        id: e.sync_uuid, branchId, category: e.category, description: e.description,
        amount: e.amount, actorName: e.actor_name, createdAt: e.created_at,
      });
    }

    if (branches.length === 0 && categories.length === 0 && products.length === 0 && sales.length === 0
        && movements.length === 0 && expenses.length === 0 && deletedProducts.length === 0) {
      return { salesPushed: 0, movementsPushed: 0, skipped };
    }

    const resp = await fetchJson<PushResponse>(`${config.serverUrl}/api/sync/push`, config.deviceApiKey, {
      method: "POST",
      body: JSON.stringify({ branches, categories, products, deletedProducts, sales, stockMovements: movements, expenses }),
    });
    ...
    for (const serverId of deletedProducts) {
      await db.runAsync("UPDATE products SET pending_sync = 0 WHERE server_id = ?", [serverId]);
    }
    ...
    const mergeDuplicate = async (
      table: "branches" | "categories" | "products", clientId: string, serverId: string
    ) => {
      const canonical = await db.getFirstAsync<{ id: number }>(
        `SELECT id FROM ${table} WHERE server_id = ?`, [serverId]
      );
      const dup = await db.getFirstAsync<{ id: number }>(
        `SELECT id FROM ${table} WHERE sync_uuid = ?`, [clientId]
      );
      if (!dup) return; // shouldn't happen — we just pushed this row by its own sync_uuid
      if (!canonical || canonical.id === dup.id) {
        await db.runAsync(`UPDATE ${table} SET server_id = ? WHERE id = ?`, [serverId, dup.id]);
        return;
      }

      if (table === "categories") {
        await db.runAsync("UPDATE products SET category_id = ? WHERE category_id = ?", [canonical.id, dup.id]);
        ...
        const dupStock = await db.getAllAsync<{ branch_id: number; stock_qty: number; low_stock_threshold: number }>(
          "SELECT branch_id, stock_qty, low_stock_threshold FROM branch_stock WHERE product_id = ?", [dup.id]
        );
        for (const s of dupStock) {
          const existing = await db.getFirstAsync<{ id: number; stock_qty: number }>(
            "SELECT id, stock_qty FROM branch_stock WHERE branch_id = ? AND product_id = ?", [s.branch_id, canonical.id]
          );
          if (existing) {
            await db.runAsync("UPDATE branch_stock SET stock_qty = ? WHERE id = ?", [existing.stock_qty + s.stock_qty, existing.id]);
          } else {
            await db.runAsync(
        ...
        await db.runAsync("DELETE FROM branch_stock WHERE product_id = ?", [dup.id]);
        // These have no uniqueness tied to product_id — a plain repoint
        // preserves history instead of losing it.
        for (const t of ["sale_items", "stock_movements", "purchase_items", "refund_items"] as const) {
          await db.runAsync(`UPDATE ${t} SET product_id = ? WHERE product_id = ?`, [canonical.id, dup.id]);
        }
      } else {
        // table === "branches" — same additive-merge idea for branch_stock,
        // plain repoint for everything else branch-scoped.
        const dupStock = await db.getAllAsync<{ product_id: number; stock_qty: number; low_stock_threshold: number }>(
          "SELECT product_id, stock_qty, low_stock_threshold FROM branch_stock WHERE branch_id = ?", [dup.id]
        );
        for (const s of dupStock) {
          const existing = await db.getFirstAsync<{ id: number; stock_qty: number }>(
            "SELECT id, stock_qty FROM branch_stock WHERE branch_id = ? AND product_id = ?", [canonical.id, s.product_id]
          );
          if (existing) {
            await db.runAsync("UPDATE branch_stock SET stock_qty = ? WHERE id = ?", [existing.stock_qty + s.stock_qty, existing.id]);
          } else {
        ...
        await db.runAsync("DELETE FROM branch_stock WHERE branch_id = ?", [dup.id]);
        for (const t of ["sales", "stock_movements", "expenses", "purchases"] as const) {
          await db.runAsync(`UPDATE ${t} SET branch_id = ? WHERE branch_id = ?`, [canonical.id, dup.id]);
        }
        ...
        await db.runAsync("UPDATE sync_config SET bound_branch_id = ? WHERE bound_branch_id = ?", [canonical.id, dup.id]);
      }
    ...
    await db.withTransactionAsync(async () => {
      ...
      for (const uuid of resp.acceptedSaleIds) {
        await db.runAsync("UPDATE sales SET synced_at = ? WHERE sync_uuid = ?", [now, uuid]);
      }
      for (const uuid of resp.acceptedMovementIds) {
        await db.runAsync("UPDATE stock_movements SET synced_at = ? WHERE sync_uuid = ?", [now, uuid]);
      }
      for (const uuid of resp.acceptedExpenseIds) {
        await db.runAsync("UPDATE expenses SET synced_at = ? WHERE sync_uuid = ?", [now, uuid]);
      }
      ...
      await db.runAsync("UPDATE sync_config SET last_push_at = ? WHERE id = 1", [now]);
    });

    return { salesPushed: resp.acceptedSaleIds.length, movementsPushed: resp.acceptedMovementIds.length, skipped };
  },

  // Convenience for the "Sync Now" button — push first (so this device's
  // own changes are on the server before pulling), then pull.
  syncNow: async (): Promise<{ salesPushed: number; movementsPushed: number; skipped: number; pulled: number }> => {
    const pushResult = await syncRepo.push();
    const pullResult = await syncRepo.pull();
    return {
      ...pushResult,
      pulled: pullResult.branches + pullResult.categories + pullResult.products + pullResult.stockRows
        + pullResult.salesReceived + pullResult.movementsReceived + pullResult.expensesReceived,
    };
  },
  ...
  setAdminKey: async (adminKey: string): Promise<void> => {
    ...
    if (!config) throw new SyncError("NOT_CONNECTED");
    const db = await getDb();
    await db.runAsync("UPDATE sync_config SET admin_key = ? WHERE id = 1", [adminKey.trim() || null]);
  },
  ...
  createDevice: async (name: string, branchId: number): Promise<{ apiKey: string }> => {
    const config = await getConfig();
    if (!config) throw new SyncError("NOT_CONNECTED");
    ...
    const created = await fetchJsonAdmin<{ deviceId: string; apiKey: string }>(
      `${config.serverUrl}/api/devices`, config.adminKey, {
        method: "POST",
  ...
  createBranch: async (name: string, address?: string, phone?: string): Promise<void> => {
    const config = await getConfig();
    if (!config) throw new SyncError("NOT_CONNECTED");
    if (!config.adminKey) throw new SyncError("NO_ADMIN_KEY");
  ...
  getRemoteSales: async (limit = 200): Promise<RemoteSale[]> => {
    ...
    const rows = await db.getAllAsync<{
      id: number; branch_name: string | null; total: number; payment_method: string;
      cashier_name: string | null; customer_name: string | null; created_at: string;
    }>("SELECT id, branch_name, total, payment_method, cashier_name, customer_name, created_at FROM remote_sales ORDER BY created_at DESC LIMIT ?", [limit]);
    if (rows.length === 0) return [];

    const ids = rows.map(r => r.id);
    const placeholders = ids.map(() => "?").join(",");
    const itemRows = await db.getAllAsync<{ remote_sale_id: number; product_name: string; unit_price: number; qty: number; subtotal: number }>(
      `SELECT remote_sale_id, product_name, unit_price, qty, subtotal FROM remote_sale_items WHERE remote_sale_id IN (${placeholders})`, ids
    );

    return rows.map(r => ({
      id: r.id, branchName: r.branch_name, total: r.total, paymentMethod: r.payment_method,
      cashierName: r.cashier_name, customerName: r.customer_name, createdAt: r.created_at,
      items: itemRows.filter(i => i.remote_sale_id === r.id).map(i => ({
        productName: i.product_name, unitPrice: i.unit_price, qty: i.qty, subtotal: i.subtotal,
      })),
    }));
  },

  getRemoteStockMovements: async (limit = 200): Promise<RemoteStockMovement[]> => {
    const db = await getDb();
    const rows = await db.getAllAsync<{
      id: number; product_name: string; branch_name: string | null; change_qty: number; reason: string;
      resulting_stock_qty: number; actor_name: string | null; created_at: string;
    }>("SELECT * FROM remote_stock_movements ORDER BY created_at DESC LIMIT ?", [limit]);
    return rows.map(r => ({
      id: r.id, productName: r.product_name, branchName: r.branch_name, changeQty: r.change_qty,
      reason: r.reason, resultingStockQty: r.resulting_stock_qty, actorName: r.actor_name, createdAt: r.created_at,
    }));
  },
  ...
  getRemoteExpenses: async (limit = 200): Promise<RemoteExpense[]> => {
    ...
    const rows = await db.getAllAsync<{
      id: number; category: string; description: string | null; amount: number;
      branch_name: string | null; actor_name: string | null; created_at: string;
    }>("SELECT * FROM remote_expenses ORDER BY created_at DESC LIMIT ?", [limit]);
    return rows.map(r => ({
      id: r.id, category: r.category, description: r.description, amount: r.amount,
      branchName: r.branch_name, actorName: r.actor_name, createdAt: r.created_at,
    }));
  },
};
