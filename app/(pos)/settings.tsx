...
import {
  ...
  StyleSheet, SafeAreaView, ScrollView, StatusBar, Platform
} from "react-native";
...
const NavRow = ({
  ...
  item: NavItem; last: boolean; s: Styles;
}) => (
  <TouchableOpacity
  ...
  >
    ...
    <Ionicons name="chevron-forward" size={17} color={s.chevron.color as string} />
  </TouchableOpacity>
);

const NavCard = ({ title, items, s }: { title?: string; items: NavItem[]; s: Styles }) => (
  <View style={s.card}>
    {!!title && <Text style={s.section}>{title}</Text>}
    {items.map((item, i) => (
      <NavRow key={item.route} item={item} last={i === items.length - 1} s={s} />
    ))}
  </View>
);
...
export default function SettingsScreen() {
  ...
  const generalItems: NavItem[] = [
    { icon: "print-outline", label: t("settings.printer"), route: "/(pos)/printer-settings", tint: C.accent },
    { icon: "time-outline",  label: t("shift.title"),      route: "/(pos)/shift",            tint: C.info   },
    { icon: "cube-outline",  label: t("settings.products"), route: "/(pos)/products-manage", tint: C.success },
    { icon: "cloud-outline", label: t("sync.title"),       route: "/(pos)/sync-settings",    tint: C.warning },
  ];

  const managementItems: NavItem[] = [
    { icon: "receipt-outline",        label: t("salesHistory.title"),     route: "/(pos)/sales-history",     tint: C.accent  },
    { icon: "bar-chart-outline",      label: t("reports.title"),          route: "/(pos)/reports",           tint: C.info    },
    { icon: "layers-outline",         label: t("inventory.title"),        route: "/(pos)/inventory",         tint: C.success },
    { icon: "people-circle-outline",  label: t("settings.customers"),     route: "/(pos)/customers-manage",  tint: C.accent  },
    { icon: "people-outline",         label: t("settings.manageUsers"),   route: "/(pos)/users-manage",      tint: C.info    },
    { icon: "business-outline",       label: t("branches.title"),         route: "/(pos)/branches-manage",   tint: C.accent  },
    { icon: "cart-outline",           label: t("suppliers.title"),        route: "/(pos)/suppliers-manage",  tint: C.success },
    { icon: "bag-handle-outline",     label: t("purchases.title"),        route: "/(pos)/purchases",         tint: C.info    },
    { icon: "cash-outline",           label: t("expenses.title"),         route: "/(pos)/expenses",          tint: C.warning },
    { icon: "warning-outline",        label: t("alerts.title"),           route: "/(pos)/alerts",            tint: C.danger  },
    { icon: "storefront-outline",     label: t("settings.shopSettings"),  route: "/(pos)/shop-settings",     tint: C.accent  },
    { icon: "archive-outline",        label: t("backup.title"),           route: "/(pos)/backup",            tint: C.muted   },
  ];
      ...
      <ScrollView contentContainerStyle={s.scrollOuter}>
        <View style={[s.scroll, isTablet && s.scrollTablet]}>
          ...
          <View style={s.profileCard}>
            <View style={s.avatar}>
              <Text style={s.avatarLetter}>{(user?.name ?? "?").slice(0, 1).toUpperCase()}</Text>
            </View>
            <View style={{ flex: 1 }}>
            ...
            <TouchableOpacity style={s.profileAction} onPress={() => router.push("/(pos)/change-password")}>
              <Ionicons name="key-outline" size={16} color={C.accent} />
              <Text style={s.profileActionText}>{t("settings.changePassword")}</Text>
            </TouchableOpacity>
          </View>

          <NavCard title={t("settings.general")} items={generalItems} s={s} />

          {admin && <NavCard title={t("settings.management")} items={managementItems} s={s} />}
          ...
          <View style={s.card}>
            ...
            <View style={s.optionRow}>
              {THEME_OPTIONS.map(opt => (
                <TouchableOpacity
                  ...
                  activeOpacity={0.8}
                >
                  <Text style={[s.optionText, mode === opt.key && s.optionActiveText]}>
                    {opt.label}
                  </Text>
                </TouchableOpacity>
              ))}
            ...
            <Text style={[s.optionLabel, { marginTop: 16 }]}>{t("settings.language")}</Text>
            <View style={s.optionRow}>
              {LANG_OPTIONS.map(opt => (
                <TouchableOpacity
                  ...
                  activeOpacity={0.8}
                >
                  <Text style={[s.optionText, lang === opt.key && s.optionActiveText]}>
                    {opt.label}
                  </Text>
                </TouchableOpacity>
              ))}
...
const makeStyles = (C: ThemeColors, isTablet: boolean) => StyleSheet.create({
  ...
  header:  { flexDirection: "row", alignItems: "center", gap: 12, marginBottom: 2, paddingTop: ANDROID_STATUS_BAR + (isTablet ? 6 : 10) },
  backBtn: { width: 36, height: 36, borderRadius: R.md, backgroundColor: C.surface,
  ...
  profileCard: { flexDirection: "row", alignItems: "center", gap: 12,
                 backgroundColor: C.surface, borderRadius: R.xl, padding: 14,
                 borderWidth: 1, borderColor: C.border, ...Shadow.sm },
  avatar:      { width: 46, height: 46, borderRadius: 23, backgroundColor: C.accent,
                 alignItems: "center", justifyContent: "center" },
  avatarLetter:{ color: C.accentFg, fontSize: F.lg, fontWeight: "800" },
  profileName: { color: C.text, fontSize: F.lg, fontWeight: "800" },
  profileRole: { color: C.muted, fontSize: F.xs, marginTop: 2, textTransform: "uppercase", letterSpacing: 0.6, fontWeight: "700" },
  profileAction:     { flexDirection: "row", alignItems: "center", gap: 5,
                       backgroundColor: C.accentSoft, paddingHorizontal: 10, paddingVertical: 8, borderRadius: R.full },
  profileActionText: { color: C.accent, fontSize: F.xs, fontWeight: "700" },

  card:    { backgroundColor: C.surface, borderRadius: R.xl, paddingHorizontal: 14, paddingVertical: 6,
             borderWidth: 1, borderColor: C.border, ...Shadow.sm },
  section: { color: C.muted, fontSize: F.xs, fontWeight: "800",
             textTransform: "uppercase", letterSpacing: 0.8, marginTop: 12, marginBottom: 4 },

  navRow:        { flexDirection: "row", alignItems: "center", gap: 12, paddingVertical: 11 },
  navRowDivider: { borderBottomWidth: 1, borderBottomColor: C.border },
  navIcon:       { width: 32, height: 32, borderRadius: R.md, alignItems: "center", justifyContent: "center" },
  ...
  optionLabel: { color: C.textSub, fontSize: F.sm, fontWeight: "700", marginBottom: 8 },
  optionRow: { flexDirection: "row", gap: 8, marginBottom: 12 },
  optionBtn: { flex: 1, paddingVertical: 12, borderRadius: R.md,
  ...
  logoutBtn:     { flexDirection: "row", gap: 8, backgroundColor: C.surface, borderRadius: R.lg,
                   paddingVertical: 14, alignItems: "center", justifyContent: "center",
                   borderWidth: 1, borderColor: C.border },
  logoutBtnText: { color: C.danger, fontSize: F.md, fontWeight: "800" },

  versionText: { color: C.muted, fontSize: F.xs, textAlign: "center", marginTop: 2 },
});
