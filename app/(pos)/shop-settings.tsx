      ...
      <ScrollView contentContainerStyle={s.scrollOuter}>
        <View style={[s.scroll, isTablet && s.scrollTablet]}>
          ...
          <View style={s.card}>
            ...
            <Field s={s} C={C} label={t("shopSettings.name")} value={name} onChangeText={setName} />
            <Field s={s} C={C} label={t("shopSettings.address")} value={address} onChangeText={setAddress} multiline />
            <Field s={s} C={C} label={t("shopSettings.phone")} value={phone} onChangeText={setPhone} keyboardType="phone-pad" />
            <Field s={s} C={C} label={t("shopSettings.currency")} value={currency} onChangeText={setCurrency} placeholder="$" autoCapitalize="characters" />
          </View>

          <View style={s.card}>
            <Text style={s.section}>{t("shopSettings.charges")}</Text>
            <Field s={s} C={C} label={t("shopSettings.tax")} value={taxPercent} onChangeText={setTaxPercent} keyboardType="numeric" />
          </View>

          <View style={s.card}>
            <Text style={s.section}>{t("shopSettings.receipt")}</Text>
            <Field s={s} C={C} label={t("shopSettings.receiptHeader")} value={receiptHeader} onChangeText={setReceiptHeader} multiline />
            <Field s={s} C={C} label={t("shopSettings.receiptFooter")} value={receiptFooter} onChangeText={setReceiptFooter} multiline />
          </View>

          <View style={s.card}>
            <Text style={s.section}>{t("shopSettings.labelPrinting")}</Text>
            <Field s={s} C={C} label={t("shopSettings.labelDefaultText")} value={labelDefaultText} onChangeText={setLabelDefaultText} />
            <Text style={s.loyaltyHint}>{t("shopSettings.labelDefaultTextHint")}</Text>
          </View>

          <View style={s.card}>
            <View style={s.loyaltyHeaderRow}>
              <Text style={[s.section, { marginBottom: 0 }]}>{t("shopSettings.loyalty")}</Text>
              <Switch
            ...
            {loyaltyEnabled && (
              <>
                <Field s={s} C={C} label={t("shopSettings.loyaltyEarnRate")} value={loyaltyEarnRate} onChangeText={setLoyaltyEarnRate} keyboardType="numeric" />
                <Field s={s} C={C} label={t("shopSettings.loyaltyRedeemRate")} value={loyaltyRedeemRate} onChangeText={setLoyaltyRedeemRate} keyboardType="numeric" />
                <Text style={s.loyaltyHint}>{t("shopSettings.loyaltyHint", { currency: currency || "$" })}</Text>
              </>
            )}
          </View>
...
const Field = ({
  s, C, label, value, onChangeText, placeholder, multiline, keyboardType, autoCapitalize,
}: {
  s: Styles; C: ThemeColors; label: string; value: string; onChangeText: (v: string) => void;
...
const makeStyles = (C: ThemeColors, isTablet: boolean) => StyleSheet.create({
  ...
  loyaltyHeaderRow: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: 12 },
  ...
  saveBtn:     { flexDirection: "row", gap: 8, backgroundColor: C.accent, borderRadius: R.lg,
