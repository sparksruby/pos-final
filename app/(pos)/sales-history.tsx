...
import { periodBounds, periodLabel, shiftAnchor, isCurrentPeriod, type Period } from "../../src/utils/reportPeriods";
...
export default function SalesHistoryScreen() {
  ...
  const loadSales = () => {
    ...
    return salesRepo.getSales(from.toISOString(), to.toISOString(), currentBranchId ?? undefined)
      .then(setSales)
      .finally(() => setIsLoading(false));
  };

  useEffect(() => { loadSales(); }, [period, anchor, currentBranchId]);
  ...
  const setRefundQty = (saleItemId: number, remaining: number, qty: number) => {
    setRefundQtys(prev => ({ ...prev, [saleItemId]: Math.max(0, Math.min(qty, remaining)) }));
  };
  ...
  const confirmRefund = async () => {
    ...
    try {
      ...
      alert(
        ...
        t("salesHistory.refundSuccessMsg", { amount: `${currency}${refund.amount.toLocaleString()}` })
      );
    } catch (e: any) {
      if (e instanceof InvalidRefundError && e.message.startsWith("OVER_REFUND")) {
        const [, name, available] = e.message.split(":");
        alert(t("common.error"), t("salesHistory.refundOverAvailable", { name, available }));
      } else {
  ...
  const chooseExportMethod = (onChoose: (method: "share" | "save") => void) => {
    if (!isDirectSaveAvailable) { onChoose("share"); return; }
  ...
  const handleSavePdf = (sale: Sale) => chooseExportMethod(async method => {
    setPdfId(sale.id);
    try {
      const ok = await exportReceiptPdf(sale, shopSettings, method);
      if (method === "save" && ok) alert(t("common.saved"), t("common.savedToFolder"));
    } catch {
  ...
  const summary = useMemo(() => {
    const refunded = sales.reduce((sum, sale) => sum + (sale.refundedAmount ?? 0), 0);
    const revenue = sales.reduce((sum, sale) => sum + sale.total, 0) - refunded;
    const productTotals = new Map<string, number>();
    for (const sale of sales) {
      for (const item of sale.items) {
        productTotals.set(item.productName, (productTotals.get(item.productName) ?? 0) + item.qty);
      }
    }
  ...
  const filteredSales = searchQuery.length === 0
    ? sales
    : sales.filter(sale =>
        ...
        sale.items.some(item => item.productName.toLowerCase().includes(searchQuery))
      );
  ...
  useEffect(() => { setVisibleCount(PAGE_SIZE); }, [period, anchor, search]);
  ...
  const handleExport = () => chooseExportMethod(async method => {
    setIsExporting(true);
    try {
      const ok = await exportSalesExcel({
        sales, periodLabel: label, currency,
        totalRevenue: summary.revenue, topProducts: summary.topProducts,
      }, method);
      if (method === "save" && ok) alert(t("common.saved"), t("common.savedToFolder"));
    } catch {
  ...
  return (
    <SafeAreaView style={s.root}>
      ...
      <View style={s.navRow}>
        <TouchableOpacity style={s.navBtn} onPress={() => setAnchor(a => shiftAnchor(period, a, -1))}>
      ...
      {isLoading ? (
        ...
        <ScrollView contentContainerStyle={s.scroll}>
          <View style={s.summaryRow}>
            ...
            <View style={s.summaryCard}>
              <Text style={s.summaryValue}>{currency}{summary.revenue.toLocaleString()}</Text>
              <Text style={s.summaryLabel}>{t("salesHistory.totalRevenue")}</Text>
            </View>
            {summary.refunded > 0 && (
              <View style={s.summaryCard}>
                <Text style={[s.summaryValue, { color: C.danger }]}>-{currency}{summary.refunded.toLocaleString()}</Text>
                <Text style={s.summaryLabel}>{t("salesHistory.totalRefunded")}</Text>
              </View>
            )}
          </View>
            ...
            <View style={s.card}>
              ...
              {summary.topProducts.map(([name, qty]) => (
                <View key={name} style={s.topRow}>
                  ...
                  <Text style={s.topQty}>{t("salesHistory.qtySold", { qty })}</Text>
                </View>
              ))}
            </View>
          )}
              ...
              >
                <View style={s.saleRow}>
                  <View style={s.saleIcon}>
                    <Ionicons name={METHOD_ICONS[sale.paymentMethod] ?? "receipt-outline"} size={16} color={C.accent} />
                  </View>
                  <View style={{ flex: 1 }}>
                    <Text style={s.saleTime}>{new Date(sale.createdAt).toLocaleString()}</Text>
                    <Text style={s.saleSub}>
                      {t("salesHistory.items", { count: sale.items.length })}
                      {branchCount > 1 && sale.branchName ? ` · ${sale.branchName}` : ""}
                      {sale.customerName ? ` · ${sale.customerName}` : ""}
                    </Text>
                  </View>
                  <Text style={s.saleTotal}>{currency}{sale.total.toLocaleString()}</Text>
                  ...
                  <View style={s.saleDetail}>
                    {sale.items.map(item => (
                      <View key={item.id} style={s.detailRow}>
                        <Text style={s.detailLabel}>
                          ...
                          {item.refundedQty > 0 && (
                            <Text style={s.detailReturned}> ({t("salesHistory.returned", { qty: item.refundedQty })})</Text>
                          )}
                        </Text>
                    ...
                    {!!sale.refundedAmount && (
                      <View style={s.detailRow}>
                        <Text style={[s.detailLabel, { color: C.danger, fontWeight: "700" }]}>{t("salesHistory.refunded")}</Text>
                        <Text style={[s.detailAmt, { color: C.danger }]}>-{currency}{sale.refundedAmount.toLocaleString()}</Text>
                      </View>
                    )}
                    <View style={s.saleActionsRow}>
                      ...
                      >
                        {printingId === sale.id
                          ...
                          : <Ionicons name="print-outline" size={14} color={C.text} />
                        }
                        <Text style={s.saleActionText}>{t("salesHistory.reprint")}</Text>
                      </TouchableOpacity>
                      <TouchableOpacity
                        style={s.saleActionBtn}
                      ...
                      >
                        {pdfId === sale.id
                          ? <ActivityIndicator size="small" color={C.text} />
                          : <Ionicons name="document-outline" size={14} color={C.text} />
                        }
                        <Text style={s.saleActionText}>{t("salesHistory.savePdf")}</Text>
                      </TouchableOpacity>
                    </View>
                    {sale.items.some(i => i.qty > i.refundedQty) && (
                      <TouchableOpacity style={s.refundBtn} onPress={() => openRefund(sale)}>
                        <Ionicons name="return-up-back-outline" size={14} color={C.danger} />
                        <Text style={s.refundBtnText}>{t("salesHistory.refund")}</Text>
                      </TouchableOpacity>
                    )}
                  </View>
                )}
      ...
      {/* Refund */}
      <Modal visible={!!refundSale} animationType="slide" transparent onRequestClose={() => setRefundSale(null)}>
        <View style={s.modalOverlay}>
          {refundSale && (
            <View style={s.modalSheet}>
                ...
                {refundSale.items.map(item => {
                  ...
                  return (
                    <View key={item.id} style={s.refundItemRow}>
                      <View style={{ flex: 1 }}>
                        <Text style={s.refundItemName} numberOfLines={1}>{item.productName}</Text>
                        <Text style={s.refundItemSub}>{t("salesHistory.refundAvailable", { qty: remaining })}</Text>
                      </View>
                      <View style={s.qtyRow}>
                        <TouchableOpacity style={s.qtyBtn} onPress={() => setRefundQty(item.id, remaining, qty - 1)}>
              ...
              <Text style={[s.label, { marginTop: 10 }]}>{t("salesHistory.refundReason")}</Text>
              <TextInput
                style={s.input}
                value={refundReason}
              ...
              <View style={s.modalActions}>
                <TouchableOpacity style={s.cancelBtn} onPress={() => setRefundSale(null)}>
                  <Text style={s.cancelBtnText}>{t("common.cancel")}</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  style={[s.confirmRefundBtn, Object.values(refundQtys).every(q => q <= 0) && s.saveBtnOff]}
                  onPress={confirmRefund}
                  disabled={Object.values(refundQtys).every(q => q <= 0) || refunding}
                >
                  {refunding
                    ? <ActivityIndicator color="#fff" />
                    : <Text style={s.confirmRefundText}>{t("salesHistory.refund")}</Text>
                  }
                </TouchableOpacity>
              </View>
            </View>
...
const makeStyles = (C: ThemeColors, isTablet: boolean) => StyleSheet.create({
  ...
  exportBtn: { backgroundColor: C.accent, width: 34, height: 34, borderRadius: R.md,
               alignItems: "center", justifyContent: "center" },
  ...
  periodBtn: { flex: 1, paddingVertical: 10, borderRadius: R.md, backgroundColor: C.card,
  ...
  navLabel: { color: C.text, fontSize: F.md, fontWeight: "700", minWidth: 140, textAlign: "center" },

  scroll: { padding: 16, paddingTop: 8, gap: 12 },

  summaryRow: { flexDirection: "row", gap: 12 },
  summaryCard: { flex: 1, backgroundColor: C.surface, borderRadius: R.lg, padding: 16,
                 borderWidth: 1, borderColor: C.border, alignItems: "center", ...Shadow.sm },
  ...
  saleCard: { backgroundColor: C.surface, borderRadius: R.lg, padding: 14,
              borderWidth: 1, borderColor: C.border, ...Shadow.sm },
  saleRow: { flexDirection: "row", alignItems: "center", gap: 10 },
  saleIcon: { width: 32, height: 32, borderRadius: 16, backgroundColor: C.accentSoft,
              alignItems: "center", justifyContent: "center" },
  ...
  saleDetail: { marginTop: 10, paddingTop: 10, borderTopWidth: 1, borderTopColor: C.border, gap: 4 },
  ...
  refundBtn: { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 6,
               marginTop: 8, paddingVertical: 9, borderRadius: R.md,
               backgroundColor: C.card, borderWidth: 1, borderColor: C.danger },
  refundBtnText: { color: C.danger, fontSize: F.xs, fontWeight: "800" },

  saleActionsRow: { flexDirection: "row", gap: 8, marginTop: 10 },
  saleActionBtn: { flex: 1, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 6,
  ...
  refundItemRow: { flexDirection: "row", alignItems: "center", paddingVertical: 10,
  ...
  qtyBtn: { width: 28, height: 28, borderRadius: 14, backgroundColor: C.card,
            alignItems: "center", justifyContent: "center", borderWidth: 1, borderColor: C.border },
  ...
  confirmRefundBtn: { flex: 1, paddingVertical: 13, borderRadius: R.md, backgroundColor: C.danger,
                      alignItems: "center" },
